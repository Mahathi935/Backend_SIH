...
def respond(messages):
    """
    messages: either a list like [{'role':'user','content':'...'}] or a plain string.
    Returns: a dict with keys:
      - reply: string (what to show to the user)
      - tag: intent tag (if available)
      - precaution: optional (if provided by the model)
    """
    # Accept either a list of messages or a raw string
    if isinstance(messages, list) and messages:
        last_item = messages[-1]
        if isinstance(last_item, dict):
            last = last_item.get('content', '')
        else:
            last = str(last_item)
    else:
        last = str(messages)
    ...
    res = get_response(last)

    # Default return shape
    result = {'reply': '', 'tag': None, 'precaution': None}

    # get_response returns lists like:
    # ["name", "Hi Joe please say your age."] or
    # ["some_tag", ["response1","response2"], "SomePrecaution"]
    if isinstance(res, list):
        tag = res[0] if len(res) > 0 else None
        result['tag'] = tag

        # second element might be a list of responses or a single string
        if len(res) > 1:
            responses = res[1]
            if isinstance(responses, list):
                result['reply'] = random.choice(responses) if responses else ''
            else:
                result['reply'] = str(responses)

        # optional precaution in third position
        if len(res) > 2:
            result['precaution'] = res[2]

    else:
        # fallback: get_response returned a plain string
        result['reply'] = str(res)
